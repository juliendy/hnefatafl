...
export function AIGetNextMove(
    ...
    const initialMoves = getAllMovesForSituation(
        stones,
        AIteam,
        scoreTemplateForAI
    );
    var currentTeam = AIteam;
    var trajectories: trajectory[] = initialMoves.map(
        (i): trajectory => ({
            moves: [i],
    ...
    for (var d = 0; d < depth; d++) {
        ...
        for (var i = 0; i < trajectories.length; i++) {
            ...
            var nextSteps = getAllMovesForSituation(
                ...
                currentTeam == AIteam
                    ? scoreTemplateForAI
                    : scoreTemplateForOpponent
            );

            // add all resulting trajectories to list
            var movesFromThisTrajectory: trajectory[] = nextSteps.map(
                (singlestep) => ({
    ...
    const trajWithScore: trajectoryWithTotalScore[] = trajectories.map((t) => ({
        ...t,
        totalScore: t.moves
            .map((i) => i.scoreAfterMove)
            .reduce((curSum, item) => curSum + item),
    }));

    const bestOutcomeTrajectory = trajWithScore.reduce((curMax, curVal) =>
...
function getMoveScore(
    ...
    var ret: stoneMoveWithScore[] = moves.map((m) => ({
        from: m.from,
        to: m.to,
        scoreAfterMove: 0,
        stonesAfterMove: stones,
    }));

    const gameValueBeforeMove = getBoardValue(stones);
    for (var i = 0; i < moves.length; i++) {
        var stonesAfterMove = getStonesAfterMovement(
            stones,
            moves[i].from,
            moves[i].to
        );
...
function getAllPossibleMoves(
    ...
    const boardLength: Number[] = Array.from(
    ...
    for (var i = 0; i < myStones.length; i++) {
        var horizontalMoves = boardLength.map(
            (item): Stone => ({ row: item.valueOf(), col: myStones[i].col })
        );
        var verticalMoves = boardLength.map(
            (item): Stone => ({ row: myStones[i].row, col: item.valueOf() })
        );
        var possibleTargetStones: Stone[] = horizontalMoves
